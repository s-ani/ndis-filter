...
#pragma NDIS_INIT_FUNCTION(BasicNdisFilterRegisterDevice)


_IRQL_requires_max_(PASSIVE_LEVEL)
NDIS_STATUS
BasicNdisFilterRegisterDevice(
    VOID
    )
{
    ...
    DEBUGP(DL_TRACE, "==>BasicNdisFilterRegisterDevice\n");
   
    
    NdisZeroMemory(DispatchTable, (IRP_MJ_MAXIMUM_FUNCTION+1) * sizeof(PDRIVER_DISPATCH));
    
    DispatchTable[IRP_MJ_CREATE] = BasicNdisFilterDispatch;
    DispatchTable[IRP_MJ_CLEANUP] = BasicNdisFilterDispatch;
    DispatchTable[IRP_MJ_CLOSE] = BasicNdisFilterDispatch;
    DispatchTable[IRP_MJ_DEVICE_CONTROL] = BasicNdisFilterDeviceIoControl;
    ...
    DEBUGP(DL_TRACE, "<==BasicNdisFilterRegisterDevice: %x\n", Status);
...
_Use_decl_annotations_                
NTSTATUS
BasicNdisFilterDeviceIoControl(
    PDEVICE_OBJECT        DeviceObject,
    PIRP                  Irp
    )
{
